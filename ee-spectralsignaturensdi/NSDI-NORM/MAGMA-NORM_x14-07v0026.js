...
// Enhanced with Near Real-Time (NRT) Live Wind & Atmospheric Fetching
// =========================================================================
...
// Dynamic NRT Dates (For Live Simulator & Satellite Feeds)
var nrtNow = ee.Date(Date.now());
var nrt48h = nrtNow.advance(-48, 'hour'); 
var wind12h = nrtNow.advance(-12, 'hour');

// Dates for recent footprint analysis & baselines
...
// C. Atmosphere - HISTORICAL OFFL (For Charts)
...
// D. Atmosphere - NEAR REAL-TIME NRTI (For Live Mapping)
...
// E. Historical Wind Dynamics (For Static Risk Maps & Charts)
...
// F. LIVE NRT WIND DYNAMICS (For Interactive Simulator)
var liveWindCol = ee.ImageCollection('NOAA/GFS025')
  .filterDate(wind12h, nrtNow)
  .select(['u_component_of_wind_10m_above_ground', 'v_component_of_wind_10m_above_ground']);
  
// Safely get the absolute latest GFS image
var liveWindImg = ee.Image(ee.Algorithms.If(
  liveWindCol.size().gt(0), 
  liveWindCol.limit(1, 'system:time_start', false).first(), 
  ee.Image.constant(0).rename(['u_component_of_wind_10m_above_ground', 'v_component_of_wind_10m_above_ground'])
));

var liveU = liveWindImg.select(0);
var liveV = liveWindImg.select(1);
// Convert U/V to Meteorological Direction (degrees)
var liveWindDir = liveU.atan2(liveV).multiply(180 / Math.PI).add(180).rename('Live_Direction');

// =========================================================================
// G. LOAD NEWLY REQUESTED SCIENTIFIC DATASETS (Historical Baseline)
// =========================================================================
// (Remaining datasets loaded exactly as requested...)
...
// Map Safe Timelines for Baseline Chart Plotting (Remain historically locked for trend analysis)
...
plumeSimPanel.add(ui.Label('NRT Interactive Plume Simulator', {fontWeight: 'bold', color: '#005073', margin: '0 0 5px 0'}));
plumeSimPanel.add(ui.Label('Simulating drift based on Live NOAA GFS Wind Data.', {fontSize: '11px', color: '#333', margin: '0 0 5px 0'}));

// Status label for async wind fetching
var liveStatusLabel = ui.Label('Fetching Real-Time Wind...', {fontSize: '11px', color: '#B22222', fontWeight: 'bold'});
...
// ASYNCHRONOUS EVALUATE: Fetch the exact live wind direction at the facility centroid
liveWindDir.reduceRegion({
  reducer: ee.Reducer.mean(),
  geometry: centerPt,
  scale: 10000
}).evaluate(function(result) {
  var key = Object.keys(result)[0];
  var liveAngle = result[key];
  if (liveAngle !== undefined && liveAngle !== null) {
    var roundedDir = Math.round(liveAngle);
    dirSlider.setValue(roundedDir);
    liveStatusLabel.setValue('✅ Live Wind Linked: ' + roundedDir + '°');
...
// Map Visual Layers (Notice NO2, SO2, CO, AER are now using NRT data for the last 48h)
...
createToggle('NO2 Emissions (NRT - 48hr)', nrt_no2.mean().clip(emergencyBuffer), {min: 0, max: 0.0001, palette: pal.ghg}, false, 'Proxy for fossil fuel/industrial GHG emissions.');
createToggle('SO2 Emissions (NRT - 48hr)', nrt_so2.mean().clip(emergencyBuffer), {min: 0, max: 0.0005, palette: pal.ghg}, false, 'Sulfur Dioxide tracking atmospheric residuals.');
createToggle('Carbon Monoxide (NRT - 48hr)', nrt_co.mean().clip(emergencyBuffer), {min: 0.02, max: 0.04, palette: pal.co}, false, 'Combustion exhaust and flaring index.');
createToggle('Dust & Aerosols (NRT - 48hr)', nrt_aer.mean().clip(emergencyBuffer), {min: -1, max: 2, palette: pal.dust}, false, 'UV Absorbing Aerosol Index tracking PM storms.');

// Standard Historical Baseline Layers
createToggle('Ozone (O3 - Baseline)', s5p_o3_raw.mean().clip(emergencyBuffer), {min: 0.12, max: 0.15, palette: pal.ghg}, false, 'Tropospheric ozone formation indicator.');
createToggle('Formaldehyde (HCHO - Baseline)', s5p_hcho_raw.mean().clip(emergencyBuffer), {min: 0, max: 0.0002, palette: pal.ghg}, false, 'Volatile Organic Compound (VOC) emissions proxy.');
createToggle('Methane Leaks (CH4 - Baseline)', s5p_ch4_raw.mean().clip(emergencyBuffer), {min: 1800, max: 1950, palette: pal.ch4}, false, 'Hydrocarbon and waste emission proxy.');
...
// Add Charts to Panel (Complete baseline chart listing, fully repaired)
...
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyO3, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 2000})
  .setOptions(chartOpts('Ozone Concentration (O3)', 'mol/m²', '#FF69B4')));

sidePanel.add(chartPanel);

// IMPORTANT: ADD PANEL TO INTERFACE ROOT
