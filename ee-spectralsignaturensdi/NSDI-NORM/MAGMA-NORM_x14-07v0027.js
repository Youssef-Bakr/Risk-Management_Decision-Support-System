...
// Expanded to a 48-hour rolling window and switched to mean() to prevent backend collection errors
var liveWindCol = ee.ImageCollection('NOAA/GFS025')
  .filterDate(nrt48h, nrtNow)
  .select(['u_component_of_wind_10m_above_ground', 'v_component_of_wind_10m_above_ground']);

var liveU = liveWindCol.select(0).mean();
var liveV = liveWindCol.select(1).mean();
...
// ASYNCHRONOUS EVALUATE: Null-safe fetching of live wind direction
liveWindDir.reduceRegion({
  reducer: ee.Reducer.mean(),
  geometry: centerPt,
  scale: 10000
}).evaluate(function(result) {
  if (result && Object.keys(result).length > 0 && result[Object.keys(result)[0]] != null) {
    var liveAngle = result[Object.keys(result)[0]];
    ...
    liveStatusLabel.style().set('color', '#B22222');
  }
});
...
// --- LANDSAT FIX APPLIED HERE: Using scientific band names [nir, red, green] ---
createToggle('Landsat False Color Composite', ls8_composite, {bands: ['nir', 'red', 'green'], min: 0, max: 0.3}, false, 'Landsat 8 false-color vegetation and land surface composite.');
// -------------------------------------------------------------------------------
