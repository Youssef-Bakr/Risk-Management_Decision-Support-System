...
// For 2023 Baseline Charting (Optical)
...
// D. Atmosphere, GHG, Gas & Aerosols (Sentinel-5P raw collections)
...
var noaa_wind = ee.ImageCollection("NOAA/CFSV2/FOR6H").filterBounds(emergencyBuffer).filterDate(baselineStart, baselineEnd).map(function(img) {
  var ws = img.expression('sqrt(u**2 + v**2)', {
    u: img.select('u-component_of_wind_height_above_ground'), v: img.select('v-component_of_wind_height_above_ground')
  }).rename('Wind_Speed');
  return img.addBands(ws).copyProperties(img, ['system:time_start']);
});
...
// G. Regional Surface Temperature (MODIS LST)
...
// Generate 10 Monthly Collections
...
// =========================================================================
// 5. USER INTERFACE (SIDE PANEL & 10 CHARTS)
// =========================================================================
...
createToggle('7. Facility Thermal Footprint', l9Thermal, {min: 30, max: 55, palette: pal.temp}, false, 'High-res surface temp (°C) for heat islands.');
...
createToggle('9. Atmos Chemistry (SO2)', s5p_so2_raw.mean().clip(emergencyBuffer), {min: 0, max: 0.0005, palette: pal.ghg}, false, 'Sulfur Dioxide tracking atmospheric residuals.');
createToggle('10. Wind Speed & Direction', windSpeed, {min: 0, max: 15, palette: pal.wind}, false, 'Daily surface wind velocity (m/s) (NOAA).');
createToggle('11. Flash Flood Risk Paths', flashFloodRisk, {palette: ['#00FFFF']}, false, 'Steep slope to lowland wetness transitions.');
...
chartPanel.add(ui.Label('Monthly Regional Baseline Metrics (2023)', {fontWeight: 'bold', color: '#333', margin: '0 0 15px 0'}));
...
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyNO2, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 2000}).setOptions(chartOpts('1. Nitrogen Dioxide (NO2) Emissions', 'mol/m²', '#A020F0')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyCO, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 2000}).setOptions(chartOpts('2. Carbon Monoxide (CO) Combustion', 'mol/m²', '#FF4500')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyCH4, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 2000}).setOptions(chartOpts('3. Methane (CH4) Concentration', 'ppb', '#00CED1')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlySO2, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 2000}).setOptions(chartOpts('4. Sulfur Dioxide (SO2)', 'mol/m²', '#8A2BE2')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyLST, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 1000}).setOptions(chartOpts('5. Land Surface Temp / Heat Island', 'Celsius (°C)', '#DC143C')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyAER, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 2000}).setOptions(chartOpts('6. Atmospheric Dust & Aerosols', 'Index Value', '#D2691E')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyET, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 1000}).setOptions(chartOpts('7. Regional Water Footprint (ET)', 'kg/m²', '#1E90FF')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyNDVI, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 100}).setOptions(chartOpts('8. Vegetation Health (NDVI)', 'Index Value', '#228B22')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyBSI, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 100}).setOptions(chartOpts('9. Bare Soil / Land Disturbance (BSI)', 'Index Value', '#8B4513')));
chartPanel.add(ui.Chart.image.series({imageCollection: monthlyWind, region: emergencyBuffer, reducer: ee.Reducer.mean(), scale: 25000}).setOptions(chartOpts('10. Average Wind Velocity', 'm/s', '#708090')));
