...
Parameters used: near-surface air temperature for a region of interest
Authors: Amita Mehta and Sean McCartney
--------------------------------------
Script: GDDP_Part1-Exercise_HotEvents 
--------------------------------------
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
  This exercise focuses on the following steps:
                      ...
                      -	Select SSP scenario 
                      -	Select parameter(s): daily maximum surface air temperature  
                      ...
                      - For extreme hot temperature:
                        Calculate 90th percentile value of daily maximum temperatures for summer (June-July-August - or JJS) from 2020-2100 data
                        Compare each daily image of maximum temperature with the 90th percentile value
                        Count as extreme hot event when the maximum daily temperature is warmer than the 90th percentile value
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
var myaoi = AOI
  ...
  .filter(ee.Filter.eq('ADM1_NAME', 'Damietta'));
// Center the map on myaoi
 Map.centerObject(myaoi, 12);
...
////+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//                                       Extreme Hot Summer Temperatures
////+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

//=========================================================================================================
// Summer Season Time Series of Daily Images for 2020-2100
// Summer months: June, July, August (JJA or jja)
// For Summer season select the parameter near-surface daily maximum temperatures 'tasmax'
// Clip the global dataset to your area of interest (myaoi) defined above
//========================================================================================================

// define a variable "AirT" that selects daily maximum temperature (i.e.,tasmax) from the variable "dataset" defined above
 var AirT = dataset.select('tasmax');                             
...
//======================================================================================
// Build a list of June, July, August (JJA) images using .iterate(), a method that applies an algorithm sequentially over an image collection
//======================================================================================
...
// define a variable "years" to hold an argument creating a list of years from startYear (2020) to endYear (2100) in equally-spaced increments  
 ...
 var jjaImageList = ee.List( // define a variable "jjaImageList" to store the list of processed images
...
// Filter JJA images (temperature data) and label with year. Variable "seasonImages" holds daily images for each summer season
  var seasonImages = AirT_reg.filterDate(junStart, end) // only include images between the dates specified (i.e., JJA)
...
// Convert the year list into an ImageCollection
  var jjaCollection = ee.ImageCollection(jjaImageList);
// Use print statements for the first 90 elements in this image collection to the console tab on the right
 print('JJA image count:');
 print(jjaCollection.size());
 print(jjaCollection.limit(90));

// From the summer season time series from 2020-2100, find the 90th percentile value
// The percentile values are stored in a default band named 'tasmax_p90'
 var T90perc = jjaCollection.reduce(ee.Reducer.percentile([90]));
// Define a variable "constant" to convert temperatures from Kelvin to Celsius by subtracting 273.15
// Define a new variable "T90PercC" to hold the values converted to Celsius
 var constant = 273.15;   
// Subtract the constant from the 90 percentile to convert it to  Celsius from Kelvin
 var T90percC = T90perc.subtract(constant)
...
// Add the image as a layer to the map window below, specifying the symbology and layer name 
 Map.addLayer(
  T90percC, AirTemperatureVis,
    '90th Percentile Value of Maximum Surface Air Temperature (C): 2020-2100');
    
//==============================================================================================================
// Calculate frequency of extreme hot days
// We define the days with maximum surface air temperature hotter than the 90th percentile value as extreme hot events
// - T90 collects band tasmax_p90 
// - seasonImage compares each image in jjaCollection for each year with T90 
// - BinaryImages are set to 1 at grid points where the temperature is hotter than T90
//==============================================================================================================

// define a variable "T90" to hold the selected band for the 90th percentile of maximum temperature data from JJA Collection
 var T90 = T90perc.select('tasmax_p90');

// define a variable "seasonalHotCounts" to hold the result of applying the provided function to each element in the years collection
 var seasonalHotCounts = years.map(function(year) {
 year = ee.Number(year); // ensures the year value is treated as an Earth Engine Number object

// define a variable "seasonImages" applying a filter to the "jjaCollection" keeping only images where the 'JJA_year' property equals the current year value
  var seasonImages = jjaCollection.filter(ee.Filter.eq('JJA_year', year));

// For each image, create an image so that the grid value is 1 if > T90, else 0
  var binaryImages = seasonImages.map(function(img) { // define a variable "binaryImages" applying the function to each image in the seasonImages collection
    var hot = img.gt(T90); // pixel-wise comparison between each image and the T90 threshold (Pixels = 1 [true] where temperature > T90, Pixels = 0 (false) where temperature ≥ T10)
    return hot.set('system:time_start', img.get('system:time_start')); // preserves the original image's timestamp metadata
  });

// hotCount holds Sum of 'extreme hot' days for each winter season 
  var hotCount = ee.ImageCollection(binaryImages).sum(); // Converts the binary images to an ImageCollection - each pixel value becomes the total count of cold days at that location
  hotCount = hotCount.set('JJA_year', year); // adds metadata identifying which summer year this count represents
  hotCount = hotCount.set('system:time_start', ee.Date.fromYMD(year, 1, 1)); // sets a standardized timestamp (January 1st of the year)
  return hotCount; // returns the final count image back to the mapping function
 });

// define a variable "hotFrequency" to convert the mapped results from the function above into an ImageCollection containing one hot count image for each year processed 
  var hotFrequency = ee.ImageCollection(seasonalHotCounts);

// Use print statements to return the number of images in the collection to the Console on the right
 print('Hot frequency maps:');
 print(hotFrequency.size());
...
// Add the first image in the image collection as a layer to the map window below, specifying the symbology and layer name 
 Map.addLayer(hotFrequency.first(), freqVis, 'Hot Days Count for JJA 2020');

//========================================================================================================
//                                        Plot Hot Event Time Series:
// Make time series of extreme hot events for myaoi (i.e., Nebraska, USA) and fit a trend line to this timeseries
...
var TimeSeries = ui.Chart.image.seriesByRegion({ // define a variable "TimeSeries" creating an interactive time series chart across different regions
    imageCollection: hotFrequency, // uses the multi-year hot frequency data
    regions: myaoi, // analyzes specific geographic regions (Area of Interest)
    reducer: ee.Reducer.mean(), // calculates average hot day frequency within each region
    ...
    .setOptions({
    title: 'Frequency of Days with Warmer than 90th Percentile Daily Tmax', // sets the title of the chart
