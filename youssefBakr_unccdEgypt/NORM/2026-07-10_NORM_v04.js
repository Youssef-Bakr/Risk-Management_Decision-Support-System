...
Map.addLayer(siteCenter, {color: '#d63031'}, 'FACILITY TARGET: Centralized NORM Plant Baseline [2020-01-01 to 2026-07-01]', true);
...
// --- 5. VISUAL RECONNAISSANCE & INUNDATION LAYER OVERLAYS WITH EXPLICT DATES ---
// Layer 1: SLR Compounding Hazard Map
Map.addLayer(coastalInundationExposureZone, {palette: ['#0984e3'], opacity: 0.75}, '1. RISK LAYER: Sea Level Rise Inundation Exposure Zone (<=3.5m Elevation) [2020-01-01 to 2026-07-01]', true);

// Layer 2: Hydro-Vulnerability Baseline
var mapPrecipIntensity = imergRain.select('HV_Hydrological_Rate_mm_hr').max().clip(facilityImpactZone);
Map.addLayer(mapPrecipIntensity, {min: 0, max: 12, palette: ['#ffffff', '#74b9ff', '#0984e3', '#6c5ce7']}, '2. RISK LAYER: Max Precipitation Hazard Profile (mm/hr) [2020-01-01 to 2026-07-01]', false);

// Layer 3: Exothermic Footprints
var mapThermalAnomalies = firmsThermalAnomalies.select('HV_Incinerator_Temp_K').max().clip(facilityImpactZone);
Map.addLayer(mapThermalAnomalies, {min: 300, max: 420, palette: ['#ffeaa7', '#fab1a0', '#ff7675', '#d63031']}, '3. ISR ALARM: Thermal Anomaly Hotspots (NASA FIRMS) [2020-01-01 to 2026-07-01]', true);

// Layer 4: High-Resolution Baseline Optical Imagery
var opticalComposite = s2TacticalOptical.median().clip(facilityImpactZone);
Map.addLayer(opticalComposite, {bands: ['B4', 'B3', 'B2'], min: 0, max: 2500}, '4. RECON: True-Color Tactical Optical ISR (Sentinel-2 10m) [2020-01-01 to 2026-07-01]', false);

// Layer 5: Active Microwave Synthetic Aperture Radar (SAR) Structural Overlay
var sarComposite = s1TacticalRadar.median().clip(facilityImpactZone);
Map.addLayer(sarComposite, {bands: ['VV', 'VH', 'VV'], min: -22, max: -3}, '5. RECON: SAR Structural Integrity & Roughness Profile (Sentinel-1) [2020-01-01 to 2026-07-01]', true);

// --- 6. SAFETY ENGINEERING CONTROL DASHBOARD INTERFACE ---
var dashboardPanel = ui.Panel({style: {width: '600px', padding: '18px', backgroundColor: '#ffffff', borderLeft: '3px solid #2d3436'}});
dashboardPanel.add(ui.Label('NORM FACILITY QUANTITATIVE RISK & ISR CONTROL DASHBOARD', {fontWeight: 'bold', fontSize: '16px', color: '#2d3436'}));
dashboardPanel.add(ui.Label('CRITICAL INFRASTRUCTURE SURVEILLANCE ENGINE // SYSTEM PERIOD: 2020-01-01 TO 2026-07-01', {fontSize: '11px', fontWeight: 'bold', color: '#c0392b', margin: '0 0 15px 0'}));

// Standard Chart Ingestion Utility
function generateSafetyCriticalChart(dataset, assetBand, reportTitle, horizontalLabel, verticalLabel, colorHex, renderType, baseYLimit, riskDossierText) {
  var analyticalChart = ui.Chart.image.series({
    ...
    title: reportTitle,
    vAxis: { title: verticalLabel, viewWindow: {min: baseYLimit}, titleTextStyle: {bold: true, fontSize: 10, color: '#2d3436'} },
    hAxis: { title: horizontalLabel, format: 'MM/yyyy', gridlines: {count: 0}, titleTextStyle: {bold: true, fontSize: 10, color: '#2d3436'} },
  ...
  var safetyTarpBox = ui.Panel({style: {padding: '10px', margin: '0 0 20px 0', border: '1px solid #d63031', backgroundColor: '#fdf1f1'}});
  safetyTarpBox.add(ui.Label('HAZARD DATA BRIEF & CONTROL PROTOCOLS (TARP):', {fontSize: '10px', fontWeight: 'bold', color: '#d63031', margin: '0 0 4px 0'}));
...
// Deploy Safety Charts with Explicit Dates and Targeted Metadata
generateSafetyCriticalChart(seriesPrecipMax, 'HV_Hydrological_Rate_mm_hr', 
  'CHART 1. HYDROLOGICAL HAZARD: Flash Inundation Velocity Peak [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Peak Flux Rate [Unit: mm/hr]', 
  '#0984e3', 'bar', 0, 
  "CRITICAL OPERATIONAL LIMIT (COL): 8.0 mm/hr. Exceeding this limit causes extreme sheet runoff over the desert surface. TARGET OUTCOME: Prevents hydro-scour, containment failures, and radiological leachate releases from Landfill Cells 1-4.");

generateSafetyCriticalChart(seriesWindDir, 'Wind_Direction_Deg', 
  'CHART 2. EXPOSURE PATHWAY: Atmospheric Vector Dispersion Angle [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Origin Heading [Unit: Degrees 0-360°]', 
  '#6c5ce7', 'scatter', 0, 
  "EXPOSURE ROUTE ANALYSIS: Dominant clustering near ~315° indicates Shamal weather conditions. Airborne particulates (Ra-226/Pb-210) follow a Southeast terrestrial trajectory, bypassing the northern coast but requiring downwind containment zones.");

generateSafetyCriticalChart(seriesWindV, 'V_Vector', 
  'CHART 3. ATMOSPHERIC DYNAMICS: Cross-Boundary Velocity Component (V-Vector) [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Velocity Amplitude [Unit: m/s]', 
  '#00b894', 'line', null, 
...
generateSafetyCriticalChart(seriesLST, 'HV_Thermal_LST_C', 
  'CHART 4. THERMAL LOADING: Structural Surface Heat Degradation Profile [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Surface Contact Temp [Unit: °C]', 
  '#e17055', 'line', null, 
...
generateSafetyCriticalChart(seriesFireT21, 'HV_Incinerator_Temp_K', 
  'CHART 5. EXOTHERMIC RISK: NASA FIRMS Core Brightness Thermal Anomaly [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Integrated Radiance [Unit: Kelvin]', 
  '#d63031', 'line', 290, 
...
generateSafetyCriticalChart(seriesFireConf, 'HV_Fire_Confidence_Pct', 
  'CHART 6. ALGORITHM VALIDATION: Signal-to-Noise Sensor Confidence Index [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Statistical Confidence [Unit: %]', 
  '#b2bec3', 'bar', 0, 
...
generateSafetyCriticalChart(seriesSAR_VV, 'VV', 
  'CHART 7. GEOTECHNICAL MONITORING: Active Microwave VV Radar Backscatter Variance [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Radar Reflection Return [Unit: dB]', 
  '#2d3436', 'line', -25, 
...
generateSafetyCriticalChart(seriesSAR_VH, 'VH', 
  'CHART 8. VOLUMETRIC DEFECT CAPTURE: Active Microwave VH Radar Backscatter Variance [2020-01-01 to 2026-07-01]', 'Timeline [Observation Steps]', 'Radar Cross-Polarization Return [Unit: dB]', 
  '#74b9ff', 'line', -30, 
...
print('  of radium-226 (Ra-226) and lead-210 (Pb-210) isotopes.');
...
print('  airborne hazard will travel away from the northern coast and settle over inland desert pipeline corridors.');
