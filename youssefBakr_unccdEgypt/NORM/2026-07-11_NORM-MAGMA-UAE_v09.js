...
// Target facility pin set to inactive for cleaner default loading
Map.addLayer(siteCenter, {color: '#d63031'}, 'Target: NORM Facility', false);
...
// --- 3. DYNAMIC TIME-SERIES LAYER INJECTION (SET TO INACTIVE FOR SPEED) ---
...
function addDynamicDatedLayers(collection, layerPrefix, visParams, showVisible) {
  ...
  ee.Dictionary({dates: dateList, ids: idList}).evaluate(function(res) {
    ...
    for (var i = 0; i < res.dates.length; i++) {
      ...
      // All dynamically injected historic layers are forced to false here
      Map.addLayer(img, visParams, layerName, false);
    }
  });
}
...
// Adding tactical layers to the map (All explicitly set to INACTIVE / false)
addDynamicDatedLayers(s2_OctNov24, 'Optical (S2) Oct-Nov 2024', optVis, false);
addDynamicDatedLayers(s1_OctNov24, 'SAR RGB (S1) Oct-Nov 2024', sarVis, false);
addDynamicDatedLayers(s2_Jul26, 'Optical (S2) July 2026', optVis, false);
addDynamicDatedLayers(s1_Jul26, 'SAR RGB (S1) July 2026', sarVis, false);

// --- 4. ANALYTICAL BASELINE LAYERS (ALL SET TO INACTIVE / FALSE) ---
Map.addLayer(chirpsRain.max().clip(facilityImpactZone), {min: 0, max: 50, palette: ['#ffffff', '#00e5ff', '#00838f', '#01579b']}, 'Chart 1: Max Precipitation Baseline (CHIRPS)', false);
Map.addLayer(s5pAER.mean().clip(facilityImpactZone), {min: -1, max: 2.0, palette: ['black', 'blue', 'purple', 'cyan', 'green', 'yellow', 'red']}, 'Chart 2: Mean Dust/Aerosol Baseline (S5P)', false);
Map.addLayer(smapSoilMoisture.mean().clip(facilityImpactZone), {min: 0, max: 25, palette: ['#d63031', '#f1c40f', '#27ae60', '#0984e3']}, 'Chart 3: Mean Soil Moisture Baseline (SMAP)', false);
Map.addLayer(era5WindField.select('Wind_Speed_ms').mean().clip(facilityImpactZone), {min: 0, max: 10, palette: ['#ffffff', '#a29bfe', '#6c5ce7', '#3742fa']}, 'Chart 4: Mean Wind Speed Proxies (ERA5)', false);
Map.addLayer(s5pCH4.mean().clip(facilityImpactZone), {min: 1750, max: 1900, palette: ['black', 'blue', 'purple', 'cyan', 'green', 'yellow', 'red']}, 'Chart 5: Mean Methane Concentration (S5P)', false);
Map.addLayer(firmsThermal.max().clip(facilityImpactZone), {min: 300, max: 500, palette: ['black', 'red', 'orange', 'yellow']}, 'Chart 6: Max Historical Fire Temp (FIRMS)', false);

// --- 5. NASA HLS OPTICAL INTEGRATION (ACTIVE & FORCED TO TOP OF STACK) ---
var hlsCol = ee.ImageCollection('NASA/HLS/HLSL30/v002')
  .filterBounds(siteCenter)
  .filterDate('2024-01-01', endDate); // Using a wide enough search net to catch the absolute latest

var hlsRecent = hlsCol.sort('system:time_start', false).first();

// Synchronously adding the layer forces it to the very top of the Layer Manager queue immediately.
// We use a temporary loading name, which will be updated asynchronously once the date is fetched.
var hlsLayer = ui.Map.Layer(hlsRecent.clip(facilityImpactZone), {bands: ['B4', 'B3', 'B2'], min: 0, max: 3000}, '★ MOST RECENT OPTICAL (HLS) | Fetching Date...', true);
Map.layers().add(hlsLayer);

hlsRecent.date().format('YYYY-MM-dd').evaluate(function(dateStr) {
  if (dateStr) {
    hlsLayer.setName('★ MOST RECENT OPTICAL (HLS) | Acq: ' + dateStr);
  } else {
    hlsLayer.setName('★ MOST RECENT OPTICAL (HLS) | Acquisition Unavailable');
  }
});

// --- 6. SAFE MONTHLY TEMPORAL AGGREGATION PATTERN FOR CHARTS ---
...
// --- 7. SAFETY ENGINEERING CONTROL DASHBOARD INTERFACE ---
...
generateSafetyCriticalChart(seriesFire, 'HV_Fire_Temp_K', 
  ...
  "Tracks historical thermal anomalies indicating potential uncontrolled fire hazards.", "FIRMS (Fire Information for Resource Management System).");

ui.root.insert(0, dashboardPanel);

// --- 8. AUTOMATED CONSOLE LOGGING ---
print('======================================================================================');
print('                OFFICIAL QUANTITATIVE RISK ASSESSMENT & SAFETY DOSSIER                ');
print('                FACILITY: CENTRALIZED NORM TREATMENT PLANT (AL RUWAIS)                ');
print('======================================================================================');
print('- SYSTEM OPTIMIZATION: All historical baseline map layers and dynamic SAR/Optical layers have been set to INACTIVE (false) by default to drastically improve browser loading speed and mitigate memory timeouts.');
print('- TOP-LAYER INJECTION (HLS): The NASA Harmonized Landsat Sentinel-2 (HLS) HLSL30 v002 dataset has been integrated. Using a synchronous ui.Map.Layer injection protocol, the most recent available HLS optical footprint has been forced to render as the sole ACTIVE layer directly at the top of the Map Layer hierarchy.');
print('======================================================================================');
