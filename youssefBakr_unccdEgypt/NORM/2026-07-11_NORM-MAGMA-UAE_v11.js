...
function addDynamicDatedLayers(collection, layerPrefix, visParams, showVisible) {
  ...
  ee.Dictionary({dates: dateList, ids: idList}).evaluate(function(res) {
    ...
    for (var i = 0; i < res.dates.length; i++) {
      ...
      // REFORMATTED: Date first, then the layer description
...
addDynamicDatedLayers(s2_OctNov24, 'S2 Optical', optVis, false);
addDynamicDatedLayers(s1_OctNov24, 'S1 SAR RGB', sarVis, false);
addDynamicDatedLayers(s2_Jul26, 'S2 Optical', optVis, false);
addDynamicDatedLayers(s1_Jul26, 'S1 SAR RGB', sarVis, false);
...
// --- 5. NASA HLS (HLSS30) OPTICAL INTEGRATION (ACTIVE & FORCED TO TOP OF STACK) ---
var hlsCol = ee.ImageCollection('NASA/HLS/HLSS30/v002') // Switched to HLSS30 (Sentinel-2 based HLS)
  .filterBounds(siteCenter)
  .filterDate('2024-01-01', endDate); 

var hlsRecent = hlsCol.sort('system:time_start', false).first();

// HLSS30 RGB bands are B04 (Red), B03 (Green), B02 (Blue)
var hlsLayer = ui.Map.Layer(hlsRecent.clip(facilityImpactZone), {bands: ['B04', 'B03', 'B02'], min: 0, max: 3000}, 'Fetching Date... | HLSS30 Optical', true);
Map.layers().add(hlsLayer);

hlsRecent.date().format('YYYY-MM-dd').evaluate(function(dateStr) {
  if (dateStr) {
    // REFORMATTED: Date placed at the absolute front of the layer label
    hlsLayer.setName(dateStr + ' | HLSS30 Optical');
  } else {
    hlsLayer.setName('Unavailable | HLSS30 Optical');
  }
});
...
print('======================================================================================');
print('- DATASET UPDATE: Transitioned from Landsat-based (HLSL30) to Sentinel-2-based NASA Harmonized Landsat Sentinel-2 (HLSS30 v002) for the top active optical layer.');
print('- LAYER RENAMING: All dynamically loaded satellite layers (HLSS30, Sentinel-1, Sentinel-2) have been reformatted so the acquisition date appears first in the Layer Manager list for rapid visual sorting.');
print('======================================================================================');
